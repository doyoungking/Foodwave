...
public class FileDataUtil {
	...
	private String uploadPath;//디렉토리정보.. 정보의 값은 주입받도록 코딩..
	//컨테이너로부터 주입받는 코드... @Inject  .. @Resource  @Autowired
	 ...
	 */
	//컨트롤러에서 매핑의 역활을 합니다...  첨부파일 다운받을때 처리해주는 코드
	@RequestMapping(value = "/download", method = RequestMethod.GET)
	@ResponseBody // 어떤 데이터를 포함하여 전송.. 어노테이션.. view지정하지 않고 바로 클라이언트 요청으로 응답. view 의 이름이 아니라 리턴타입에 따라 응답하게된다
	 ...
	 * @throws IOException
	 */
	// 클라이언트가 업로드한 첨부파일을 지정된위치(uploadPath 로 copy 하는 코드)
...
//	public String[] orfileUpload(MultipartFile[] file) throws IOException {
//		String[] files = new String[file.length];
//		for (int i = 0; i < file.length; i++) {
//			if (file[i].getOriginalFilename() != "") { // 실제 file객체가 존재한다면
//				String originalName = file[i].getOriginalFilename();// 확장자가져오기 위해서 전체파일명을 가져옴.
...
//				File target = new File(uploadPath, saveName);
//				FileCopyUtils.copy(fileData, target);
//				files[i] = file[i].getOriginalFilename();
//			}
//		}
//		return files;
//	}
