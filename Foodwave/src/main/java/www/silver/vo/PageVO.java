...
public class PageVO {
	...
	private int perPageNum=3;//페이지당 출력될 튜플의수, 표시될 페이지 수  
	//page 변수는 현재 페이지의 번호 
	private Integer page; // Integer 웹에서 받은 페이지 정보(String)가 없으면 null인데 int는 null을 저장할 수 없다. 오류방지
	private int totalCount;//전체튜플의수
	//page,otalCount,perPageNum 3개의 변수로 startNo 와 endNo 값을 자동으로 계산한다
	
	
	private int endPage;//페이지가 속한 그룹의 끝페이지 번호
